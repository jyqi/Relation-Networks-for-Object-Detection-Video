...
update_config(cur_path + '/../experiments/deformable/rfcn/cfgs/rfcn_coco_demo.yaml')
...
def main():
    ...
    for im_name in image_names:
        assert os.path.exists(cur_path + '/../demo/deformable/' + im_name), ('%s does not exist'.format('../demo/deformable/' + im_name))
        im = cv2.imread(cur_path + '/../demo/deformable/' + im_name, cv2.IMREAD_COLOR | cv2.IMREAD_IGNORE_ORIENTATION)
    ...
    arg_params, aux_params = load_param(cur_path + '/../model/deformable/' + ('rfcn_dcn_coco' if not args.rfcn_only else 'rfcn_coco'), 0, process=True)
    ...
    for idx, im_name in enumerate(image_names):
        ...
        # visualize
        im = cv2.imread(cur_path + '/../demo/deformable/' + im_name)
