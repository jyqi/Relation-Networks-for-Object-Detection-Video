...
from operator_py.repmet_module import *


class resnet_v1_101_fpn_dcn_rcnn_emb(Symbol):
    def __init__(self):
        """
    ...
    def get_symbol(self, cfg, is_train=True):
        ...
        # emb layer
        emb = mx.symbol.FullyConnected(name='emb', data=fc_new_2_relu, num_hidden=cfg.EMBEDDING_SIZE)
        emb_l2_norm = mx.symbol.L2Normalization(name='emb_l2_norm', data=emb)

        emb_cls_loss_ = mx.sym.Custom(op_type='Repmet', n=cfg.TRAIN.nshot, m=cfg.TRAIN.mway, k=cfg.TRAIN.k,
                                     emb_size=cfg.EMBEDDING_SIZE, embs=emb_l2_norm, labels=label)
        emb_cls_loss = mx.sym.MakeLoss(name='emb_cls_loss', data=emb_cls_loss_)
        ...
        if is_train:
            ...
            group = mx.sym.Group([rpn_cls_output, rpn_bbox_loss, cls_prob, bbox_loss, mx.sym.BlockGrad(rcnn_label), mx.sym.BlockGrad(emb_cls_loss)])
        else:
            ...
            group = mx.sym.Group([rois, cls_prob, bbox_pred, emb_l2_norm])
    ...
    def init_weight_rcnn(self, cfg, arg_params, aux_params):
        ...
        arg_params['emb_weight'] = mx.random.normal(0, 0.01, shape=self.arg_shape_dict['emb_weight'])
        arg_params['emb_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['emb_bias'])
